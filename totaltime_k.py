...
def func(file_name1, file_name2):
    with open(file_name1) as f:
        ...
        update = sum(lis)/len(lis)*1000
    with open(file_name2) as f:
        lis = list(map(float, f.read().splitlines()))
        query = sum(lis)/len(lis)*1000
    return update + query/100


# read data
for i in k:
    folder = 'C:/Users/Yu/Desktop/clustering result/' + data_name + '/k-' + str(i) + '/queryinterval-100'
    skmpp_query.append(func(folder + '/skmpp/updatetime.txt', folder + '/skmpp/querytime.txt'))
    cc_query.append(func(folder + '/cache/updatetime.txt', folder + '/cache/querytime.txt'))
    rcc_query.append(func(folder + '/rcc/updatetime.txt', folder + '/rcc/querytime.txt'))
    onlinecc_query.append(func(folder + '/hybrid_12/updatetime.txt', folder + '/hybrid_12/querytime.txt'))
...
# plt.show()
plt.savefig("figs/total_k/" + data_name + "_total_vs_k.pdf", dpi=600, bbox_inches='tight', transparent=True)
